<h1 align="center">Hey there! 👋 I'm Sholikin (Okin)</h1>

<p align="center">
  <em>Fullstack Developer • Software Engineer • Founder of <strong>ViMi Tech</strong></em><br/>
  <a href="https://github.com/Sholikin1941720140">GitHub</a> • 
  <a href="mailto:sholikintech@gmail.com">Email</a> • 
  <a href="https://www.linkedin.com/in/sholikin/">LinkedIn</a> • 
  <a href="https://vimitech-dev.my.id">Website</a>
</p>

---

### 🧠 About Me  
Hi! I'm **Sholikin**, a passionate **Software Engineer** based in **Malang, Indonesia**.  
I specialize in building robust and scalable web applications using **Laravel**, **React Native**, and **PostgreSQL**, and I love transforming ideas into real digital solutions.  

As the **Founder of ViMi Tech**, I lead the creation of modern software solutions — from ERP systems to e-commerce platforms — designed to empower businesses with clean code, reliable architecture, and stunning UI/UX.

---

### ⚙️ Tech Stack  
💻 **Backend:** Laravel | PHP | MySQL | PostgreSQL | MySQL | REST API  
🎨 **Frontend:** React Native | Vue | Bootstrap | TailwindCSS  
🧩 **DevOps:** VPS Deployment | Git & GitHub | Nginx  
🧠 **Other Skills:** Midtrans Integration | Pusher | IoT (ESP8266 + RFID)  

---

### 📊 GitHub Analytics  
<p align="center">
  <a href="https://github.com/Sholikin1941720140">
    <img height="170em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Sholikin1941720140&layout=compact&langs_count=8&theme=gotham"/>
    <img height="170em" src="https://github-readme-stats.vercel.app/api?username=Sholikin1941720140&show_icons=true&theme=gotham&count_private=true&line_height=30&icon_color=fff305&custom_title=GitHub%20Statistics&hide_rank=true"/>
  </a>
</p>

---

### 🚀 Highlight Projects  
- 🧾 **HRMS System** — Human Resource Management System with attendance, scheduling & KPI tracking.
- 🌍 **IDI Malang Raya** — Membership & digital management system for Ikatan Dokter Indonesia (IDI) Malang Raya.  
- 🧠 **IoT Academic Access System** — RFID-based attendance integrated with Laravel backend.  
- 💳 **Teh Khita App** — Tea e-commerce mobile app (React Native + Laravel + Midtrans Integration).  
  _Built with Laravel and MySQL, featuring member registration, license verification, and event management modules._

---

### 🤝 Connect with Me  
<p align="left">
  <a href="mailto:sholikinmuhammad140201@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=flat&logo=Gmail&logoColor=white"/> sholikintech@gmail.com</a><br>
  <a href="https://www.linkedin.com/in/sholikin/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white"/> linkedin.com/in/sholikin</a><br>
  <a href="https://vimitech.dev"><img src="https://img.shields.io/badge/ViMi%20Tech-111827?style=flat&logo=vercel&logoColor=white"/> vimitech-dev.my.id</a>
</p>

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Sholikin1941720140&color=blueviolet" alt="Profile views"/>
  <br><br>
  <em>“Building clean, efficient, and impactful technology — one line of code at a time.”</em>
</p>
